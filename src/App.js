...
// Token válido = existe, não é null, não é string "undefined", não é vazio
...
function App() {
  ...
  // Restaura sessão ao carregar — só se o token for válido de verdade
  useEffect(() => {
  ...
  // Busca dados sempre que usuario mudar (login ou restauração de sessão)
  useEffect(() => {
  ...
  async function adicionarPrato(e) {
    e.preventDefault();
    if (!nomePrato || !precoPrato) { mostrarMensagem("Preencha o nome e o preço."); return; }
    const token = localStorage.getItem("token");
    try {
  ...
  async function adicionarIngrediente(e) {
    e.preventDefault();
    if (!nomeIngrediente || !qtdIngrediente) { mostrarMensagem("Preencha o nome e a quantidade."); return; }
    const token = localStorage.getItem("token");
    try {
  ...
  const faturamento = pedidos.filter((p) => p.status === "entregue").reduce((total, p) => total + (p.total || 0), 0);
  ...
  const pedidosPaginados = pedidosFiltrados.slice((paginaPedidos - 1) * ITENS_POR_PAGINA, paginaPedidos * ITENS_POR_PAGINA);
  ...
  const cardapioPaginado = cardapioFiltrado.slice((paginaCardapio - 1) * ITENS_POR_PAGINA, paginaCardapio * ITENS_POR_PAGINA);
  ...
  const estoquePaginado = estoqueFiltrado.slice((paginaEstoque - 1) * ITENS_POR_PAGINA, paginaEstoque * ITENS_POR_PAGINA);
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => onChange(paginaAtual - 1)} disabled={paginaAtual === 1}
            className="px-3 py-1 rounded-lg border text-sm disabled:opacity-40 hover:bg-gray-100">← Anterior</button>
          <button onClick={() => onChange(paginaAtual + 1)} disabled={paginaAtual === totalPaginas}
            className="px-3 py-1 rounded-lg border text-sm disabled:opacity-40 hover:bg-gray-100">Próxima →</button>
        </div>
      </div>
    );
  }
  ...
  // Guard — se não tem usuário válido, mostra login
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-green-700 text-white px-6 py-4 flex justify-between items-center shadow">
        ...
        <div className="flex items-center gap-4">
          <span className="text-green-200 text-sm">Olá, {usuario?.nome} ({usuario?.nivel})</span>
          <button
            onClick={() => {
              localStorage.removeItem("token");
              localStorage.removeItem("nivel");
      ...
      <nav className="bg-white shadow px-6 py-2 flex gap-4">
        {[
          ...
          .filter(p => (paginasPermitidas[usuario?.nivel] || paginasPermitidas["admin"]).includes(p.id))
          .map(({ id, icone }) => (
            <button key={id} onClick={() => setPagina(id)}
              className={`px-4 py-2 rounded-lg font-medium capitalize transition ${
                pagina === id ? "bg-green-700 text-white" : "text-gray-600 hover:bg-gray-100"
              }`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {alertas.map((item) => (
                    <div key={item.id} className="bg-white rounded-lg p-3 border border-red-200">
                      ...
                      <span className="text-sm text-gray-500 ml-2">{item.quantidade} {item.unidade}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                {["todos", "recebido", "em preparo", "pronto", "entregue"].map(s => (
                  <button key={s} onClick={() => { setFiltroPedido(s); setPaginaPedidos(1); }}
                    className={`px-3 py-1 rounded-lg text-sm font-medium capitalize transition ${
                      filtroPedido === s ? "bg-green-700 text-white" : "bg-white border text-gray-600 hover:bg-gray-50"
                    }`}>
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>{["#", "Mesa", "Itens", "Total", "Status", "Ação"].map((h) => (
                  ...
                  {pedidosPaginados.length === 0 ? (
                    ...
                    pedidosPaginados.map((pedido) => (
                      <tr key={pedido.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 font-medium text-green-700">R$ {Number(pedido.total || 0).toFixed(2)}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${corStatus(pedido.status)}`}>{pedido.status}</span>
                        </td>
                        <td className="px-4 py-3">
                          {proximoStatus(pedido.status) && (
                            <button onClick={() => atualizarStatus(pedido.id, proximoStatus(pedido.status))}
        ...
        {pagina === "cardapio" && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <input type="text" placeholder="🔍 Buscar prato..." value={buscaCardapio}
            ...
            <div className="bg-white rounded-xl shadow p-6 mb-6">
              ...
              <form onSubmit={adicionarPrato} className="flex gap-4 flex-wrap">
                <input type="text" placeholder="Nome do prato" value={nomePrato} onChange={(e) => setNomePrato(e.target.value)} className="border rounded-lg px-3 py-2 text-sm flex-1 min-w-40" />
                <input type="text" placeholder="Descrição (opcional)" value={descPrato} onChange={(e) => setDescPrato(e.target.value)} className="border rounded-lg px-3 py-2 text-sm flex-1 min-w-40" />
                <input type="number" placeholder="Preço (ex: 29.90)" value={precoPrato} onChange={(e) => setPrecoPrato(e.target.value)} className="border rounded-lg px-3 py-2 text-sm w-40" step="0.01" />
                <button type="submit" className="bg-green-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-800 transition">+ Adicionar Prato</button>
              </form>
            </div>
                ...
                cardapioPaginado.map((prato) => (
                  <div key={prato.id} className="bg-white rounded-xl shadow p-4">
                    <div className="flex justify-between items-start">
                      ...
                      <button onClick={() => deletarPrato(prato.id, prato.nome)} className="text-red-400 hover:text-red-600 transition" title="Deletar">🗑️</button>
                    </div>
                    {prato.descricao && <p className="text-gray-500 text-sm mt-1">{prato.descricao}</p>}
                    <p className="text-green-700 font-bold mt-2">R$ {Number(prato.preco || 0).toFixed(2)}</p>
                  </div>
                ))
              )}
            </div>
        ...
        {pagina === "estoque" && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <input type="text" placeholder="🔍 Buscar ingrediente..." value={buscaEstoque}
            ...
            <div className="bg-white rounded-xl shadow p-6 mb-6">
              ...
              <form onSubmit={adicionarIngrediente} className="flex gap-4 flex-wrap">
                <input type="text" placeholder="Nome do ingrediente" value={nomeIngrediente} onChange={(e) => setNomeIngrediente(e.target.value)} className="border rounded-lg px-3 py-2 text-sm flex-1 min-w-40" />
                <input type="number" placeholder="Quantidade" value={qtdIngrediente} onChange={(e) => setQtdIngrediente(e.target.value)} className="border rounded-lg px-3 py-2 text-sm w-32" />
                <select value={unidadeIngrediente} onChange={(e) => setUnidadeIngrediente(e.target.value)} className="border rounded-lg px-3 py-2 text-sm w-28">
                  <option value="kg">kg</option>
                  <option value="litro">litro</option>
                  <option value="unidade">unidade</option>
                  <option value="pacote">pacote</option>
                </select>
                <button type="submit" className="bg-green-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-800 transition">+ Adicionar</button>
              </form>
            </div>
            <div className="bg-white rounded-xl shadow overflow-hidden">
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>{["#", "Ingrediente", "Quantidade", "Unidade", "Situação"].map((h) => (
                    <th key={h} className="text-left px-4 py-3 text-gray-600 text-sm">{h}</th>
                  ))}</tr>
                </thead>
                <tbody>
